Vagrant.configure('2') do |config|
  ...
  (1..num).each do | i |
    ...
    config.vm.define vm_name = "%s-%01d" % [payload['vm']['guest_name'], i] do | node |
      ...
      if i == num then
        groups["master"] = ["#{payload['vm']['guest_name']}-[1:1]"]
        groups["master:vars"] = {"host_type" => "master"}
  
        if num > 1 then
          groups["follower"] = [
            "#{payload['vm']['guest_name']}-[2:#{num}]"
          ]
          groups["follower:vars"] = {"host_type" => "follower"}
          groups["master:vars"]["followers"] = []
          (2..num).each do |x|
            groups["master:vars"]["followers"] << "#{payload['vm']['guest_name']}-#{x}"
          end
        end
